...
short parse_polygon_set(PyObject *polyset, Paths &paths)
{
...
static PyObject * clip(PyObject *self, PyObject *args)
{
  ...
  if (parse_polygon_set(polyA, subj) != 0) return NULL;
  if (parse_polygon_set(polyB, clip) != 0) return NULL;
  
  clpr.addPaths(subj, ptSubject, true);
  clpr.addPaths(clip, ptClip, true);
  clpr.Execute(oper, solution);

  /* Construct output in external function (use for offset) */
  /* Insert holes */
  /* Re-scale and build arrays */
}
